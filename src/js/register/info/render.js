    ...
    return `
		<div id="register-info-wrapper" class="register-info-wrapper">
			<form id="register-info-form" onsubmit="return false">
				<label>
					<span>昵称：</span>
					<input id="register-info-nickname" name="nickname" type="text" placeholder="昵称" valid="present" value="${ data.nickname || '' }" noOther>
				</label>
				<label>
					<span>电子邮箱：</span>
...
export default async (conf) => {
    if (conf.update) {
        ...
        if (result.code === 200) {
            conf.container.innerHTML = template(result.data);
        ...
        conf.container.innerHTML = template();
        const region = new Region({
            container: document.getElementById('register-info-address'),
            name: 'region'
        });
